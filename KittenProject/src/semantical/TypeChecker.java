...
public class TypeChecker {
	...
	/**
	 * Boolean for checking if the assert is used or not inside a test
	 */

	private boolean assertIsAllowed = false;
	...
	private TypeChecker(Type returnType, Table<TypeAndNumber> env, int varNum, ErrorMsg errorMsg, boolean assertIsAllowed) {
	...
	public TypeChecker(Type returnType, ErrorMsg errorMsg, boolean assertIsAllowed) {
		this.returnType = returnType;
	...
	public TypeChecker putVar(String var, Type type) {
		...
		return new TypeChecker(returnType,
			env.put(var, new TypeAndNumber(type, varNum)), varNum + 1, errorMsg, assertIsAllowed);
	}
	
	/**
	 * Yields the Boolean {@code assertIsAllowed} in this type-checker
	 * 
	 * @return the boolean which indicates if assert command is in a test
	 */
	
	public boolean isAssertIsAllowed() {
		return assertIsAllowed;
	}
	...
	public String getErrorPos(int pos) {
		
		return errorMsg.getErrorPos(pos);
	}
	
	public String getErrorPosForMsg(int pos) {
		
		return errorMsg.getErrorPosForMsg(pos);
	}
