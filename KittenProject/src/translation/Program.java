...
public class Program {
	...
	/**
	 * Generates the Java bytecode for all the test class types and
	 * dumps the relative {@code .class} files on the file system.
	 */

	public void generateJavaBytecodeForTests() {
		// we consider one class at the time and we generate its Java bytecode if it have tests inside
		for (ClassType clazz: ClassType.getAll())
			if(!clazz.getTests().isEmpty()){
				try {
					...
					System.out.println("Could not dump the Java bytecode for test class " + clazz);
				}
			}
	}
	...
	protected void storeBytecode(Bytecode bytecode) {
		if (bytecode instanceof FieldAccessBytecode){
			...
			for(CodeSignature c : ((CALL) bytecode).getDynamicTargets()){
				addTestandFixtureSignature(c.getDefiningClass());
			}
		}
	}
	
private void addTestandFixtureSignature(ClassType cltp) {   //controlla se devi fare la clean up
		
		sigs.addAll(cltp.fixturesLookup());
		
		sigs.addAll(cltp.testsLookup());	
	}
}