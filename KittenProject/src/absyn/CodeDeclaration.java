...
public abstract class CodeDeclaration extends ClassMemberDeclaration {
    ...
    protected void translateReferenced(Block block, Set<ClassMemberSignature> done, Set<Block> blocksDone) {
    	...
    	for (BytecodeList cursor = block.getBytecode(); cursor != null; cursor = cursor.getTail()) {
    		...
    		if (h instanceof GETFIELD){
    			...
    			addTestandFixture(((GETFIELD) h).getField().getDefiningClass(), done);
    			done.add(((GETFIELD) h).getField());
    		}
    		else if (h instanceof PUTFIELD){
    			
    			addTestandFixture(((PUTFIELD) h).getField().getDefiningClass(), done);
    			done.add(((PUTFIELD) h).getField());
    		}
    		else if (h instanceof CALL){
    			for (CodeSignature callee: ((CALL)h).getDynamicTargets()){
    				
    				addTestandFixture(callee.getDefiningClass(), done);
	...
	private void addTestandFixture(ClassType cltp, Set<ClassMemberSignature> done) {
		//System.out.println("aggiungo test e fixture della classe cltp: " + cltp );		//controllo da togliere
		for(FixtureSignature fs: cltp.fixturesLookup()){
			fs.getAbstractSyntax().translate(done);
		}
		done.addAll(cltp.fixturesLookup());
		
		for(TestSignature ts: cltp.testsLookup()){
			ts.getAbstractSyntax().translate(done);
		}
		done.addAll(cltp.testsLookup());
