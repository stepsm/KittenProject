...
public final class ClassType extends ReferenceType {
	...
	/**
	 * A map from test symbols to the set of signatures of the tests with
	 * that name. Because of overloading, more than one test might have a given name.
	 */

	private final Map<String, TestSignature> t = new HashMap<>();
	...
	public void addTest(String name, TestSignature tSig) {
		t.put(name, tSig);
	}
	...
	public Set<FixtureSignature> fixturesLookup(){
		return fixtures;
	}
	...
	public Set<TestSignature> testsLookup(){
		 
		    Set<TestSignature> ts = new HashSet<>(t.values());
